...
def test_reservation_faults_are_durable_and_attempt_ordinals_never_reuse(tmp_path: Path) -> None:
    ...
    with Project.create(path) as project:
        ...
        assert repository.get_job(second_claim.descriptor.job_id).status is JobStatus.INDETERMINATE  # type: ignore[union-attr]
        assert repository.list_attempts(second_claim.descriptor.job_id)[-1].reservation.status is (
            AttemptStatus.INDETERMINATE
        )

        retry_payload = {"job_id": second_claim.descriptor.job_id, "attempt_ordinal": 2}
        retry = RetryApprovalDescriptor(
            "retry-approval-1",
            second_claim.descriptor.job_id,
            2,
            retry_payload,
            hashlib.sha256(storage.canonical_json(retry_payload)).hexdigest(),
        )
        repository.store_retry_approval(retry, now=NOW + timedelta(seconds=12))
        assert repository.store_retry_approval(
            retry, now=NOW + timedelta(seconds=13)
        ) == repository.load_retry_approval(second_claim.descriptor.job_id, 2)
        assert repository.load_retry_approval(second_claim.descriptor.job_id, 2) is not None
