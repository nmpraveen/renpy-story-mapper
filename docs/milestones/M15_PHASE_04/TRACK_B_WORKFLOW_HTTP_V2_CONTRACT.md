...
Status: Frozen integration contract v2

Contract/version: `story-map-v2-workflow-http-v2`

Machine-readable schema:
`src/renpy_story_mapper/story_map_v2/schemas/story_map_workflow_http_v2.schema.json`

Public synthetic fixture:
`tests/fixtures/story_map_v2/phase04_workflow_http_v2.json`

Normative coherence validator/version:
`story-map-v2-workflow-http-coherence-v1` in
`src/renpy_story_mapper/story_map_v2/workflow_http_contract.py`
...
Every request or response must pass both the Draft 2020-12 schema layer and the normative coherence
validator. Schema validation owns strict shapes, command/code specialization, enums, and local
conditions. The coherence validator owns identity lineage, membership, uniqueness, exact counts,
route references, derived ceiling formulas, and recursively sanitized messages. Failure in either
layer rejects the value. Track C2's HTTP composer must call the coherence validator before returning
an envelope.
