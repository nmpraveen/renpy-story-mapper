...
| Real MsDenvers check | Orchestra with explicit user consent | Run supported workflow, inspect representative story/branch/path samples, show result to user | Integrated path and zero-submit preview | In progress | Integrated gate: 123 tests plus Ruff, strict mypy, JavaScript syntax, and whitespace passed; zero-submit website preview is next |
