...
Baseline: branch `codex/m15-phase04-full-game`, product checkpoint `f25e8c0`, draft PR #30 open and
unmerged.
...
| Local summary observability | Orchestra | Keep raw local prompt/response evidence outside Git, show compact per-query progress, and remove exact-format work from the local model | User request and demonstrated local failures | Complete | `4426a58` through `f25e8c0`; private opt-in and consolidated JSONL transcripts, 425-row website table that reloads after refresh, one-summary prompt v3, Python-owned membership/mechanics/length/review/key binding, mapping-only local publication, meaningful section labels/overview, and a bounded section index. Live result: 425 accepted, 0 failed, 0 placeholders, active generation `cf5f7daa…f3a6`, no cloud AI. |
| Final lean gate and PR readiness | Orchestra | Focused tests, user screenshots, sharded CI, one Release/package gate, evidence, ready PR #30 | User closes current checkpoint | Complete | User explicitly chose partial Phase 04 closeout and deferred mock-level semantic hierarchy/UI redesign. Current closeout passed 136 focused tests, tracked-file Ruff, strict mypy, JavaScript syntax, and whitespace. The full Release exercised 1,782 tests: 1,781 passed, one Chrome scale test skipped, 16 hardware tests deselected, and only stale web-asset hashes failed; the three hashes were corrected, the exact checksum regression passed, and exact corrected-head package build/install/import/assets passed. Push and one CI inspection are the remaining PR operations, not implementation work. |
