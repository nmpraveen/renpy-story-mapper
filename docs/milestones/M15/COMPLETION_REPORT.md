...
assembler's three-state/two-transition progression context. Failing-first input-v3/prompt-v6/
repair-policy-v7 correction projects only authority-bound `progression_id` and exact assembler
context guidance without weakening Python validation. Focused/static gates pass; independent
...
| Current whole-scope Stage H | Assembler-context correction review | V8 failed closed after two calls with `hierarchy_not_representable`; no hierarchy/publication exists. Input v3/prompt v6/repair policy v7 project authority-bound progression and exact assembler context without weakening Python validation. Focused/static gates pass; independent exact-head review is pending. |
