...
| Current whole-scope Stage H | Ambiguity-threshold correction review | V6 failed closed after two calls with `uncertain_membership`; no hierarchy/publication exists. Commit `b81dbad` distinguishes ordinary semantic ambiguity from actual missing/impossible membership without weakening Python rejection and awaits exact-head review. |
