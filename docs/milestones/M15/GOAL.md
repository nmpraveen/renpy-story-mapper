...
| 7-13 | Stage H/Stage E API/service/browser flow, batched logical/transport provenance, fake-provider fault matrix, exact manifests, durable jobs/cache/accounting, four-call ceiling, reopen and zero-submit replay | V5 manifest `consent_14655be0e14020371cdf104f` used two calls and failed closed with `semantic_reinterpretation`: 497,401 input tokens, 30,020 output tokens, no hierarchy/records/publication, no Stage E, and unchanged fingerprints. Artifact SHA-256 is `DFD1533DFD4CEE3F5DF53F5E5FD9E9EA36D398EBBAFDFBE0B18D57B83B3F5933`. Lifecycle returned to `Revise`. Commit `b97fa47` maps whole-scope repair locks explicitly while preserving Python enforcement and advances consent repair policy to v4. First exact-head review `de7c5e7` confirmed runtime enforcement but found two P2 test/evidence gaps. Nonempty locked-cluster and Stage E wrapper regressions plus the corrected consent-only identity statement now pass; expanded 107/107, Ruff, and strict mypy are clean. Repeated semantic review remains `PASS`; exact-head rereview is next. No fresh provider run may occur before rereview and a new zero-submit manifest. |
