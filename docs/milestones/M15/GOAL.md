...
Status: In progress in `Revise`; the live Stage H criterion-3 request projection is under correction.
...
open PR #26. It is not M16, M14, a second PR, or a new branch family. The prior `PR ready` claim is
revoked; its provider-free map, screenshots, visible-order export, release result, and reviews are
historical rejected-baseline evidence only.

Scope authority: `docs/MASTER_PLAN.md`, the user's 2026-07-21 instruction to execute the complete
M15.1 coordinator handoff, and the private local acceptance oracle used only after generation.

Original M15 baseline: `a447a4eefbd7c093bdb2767e62a393805af068ac`

M15.1 correction base: `55ae57406cfb07a3c088d0dfd7c3b7e04ca9a719`

Integration branch: `codex/m15-msday1-narrative-map`

Pull request: [PR #26](https://github.com/nmpraveen/renpy-story-mapper/pull/26), which must remain
...
- Honest status that never labels deterministic/provider-free structure as a completed/current
  human Story Map and never substitutes generic titles or counts-as-summaries for failed AI work.
- Synthetic/fake-provider, exact private, real-browser, compatibility, privacy, immutability,
...
15. The all-expanded normal view has five top-level sections, exactly four choice widgets, eight
    logical arm cards, three compact visual rejoin markers, zero routing-only station cards, and no
    more than 32 story-facing rows/cards; every row contributes distinct narrative meaning.
16. Narrative mode is normal-flow semantic DOM in a column no wider than 896 CSS pixels, with no
    global canvas/world transform, absolute station surface, horizontal page scroll, distant curves,
    or unreadable fit-all shrink. Initial scale is 1.0 and fit-width remains at least 0.75.
17. Real Chrome at 100% and 200% passes the specified initial/full-page/section/detail captures,
    typography, target size, spacing, disclosure, search, keyboard, connector containment/endpoint,
    non-overlap, and complete interactive-ID-to-evidence mapping assertions.
18. The final independent reviewer first freezes a source-first review without the oracle/mockups,
    then compares the same frozen candidate against them. It reads the complete private source,
    persisted map, visible order, Detail/Evidence, and screenshots and returns no unresolved P0-P2.
19. Source/archive hashes, sizes, and timestamps remain unchanged; no game/Ren'Py/creator code or
    runtime tracing executes; M10/M11/M12/M13 authority/compatibility remain unchanged except for
    intentional versioned M15.1 records; private source/oracle/handoff/mockup content never enters
    Git or provider input.
20. Focused track gates, independent exact-head track reviews, integrated fake-provider/browser/
    compatibility checks, user approval of the exact final screenshots, one final Windows Release/
...
| 7-13 | Stage H/Stage E API/service/browser flow, batched logical/transport provenance, fake-provider fault matrix, exact manifests, durable jobs/cache/accounting, four-call ceiling, reopen and zero-submit replay | In `Revise`. Provider-free work through `5d782ce` and the reviewed authority-validation/lost-CAS correction remain exact-head evidence, but live manifest `consent_c988d3b944a81c177303d32f` exhausted two calls without a hierarchy. Its first response failed validation and targeted repair ended `provider_process_failed`; Stage E was untouched. The request audit then proved criterion 3 was not implemented: 732 unit IDs were sent with zero of 741 available evidence records and without required speaker, source, or full structural-ownership context. Failing-first correction, independent exact-head review, repeated semantic PASS, and fresh provider-free gates are required. Any later Stage H transmission needs a new exact manifest, explicit manifest consent, and explicit approval to exceed four total Day 1 submissions. |
...
| 19 | Pre/post source/archive fingerprints, provider request-envelope/file-access audit, staged/private-content scans, compatibility bytes and no-execution counters | Preflight and both exhausted Stage H runs preserve source/archive hashes. Provider-free integrated scan is clean; synthetic acceptance records `provider_calls: 0` and `game_execution_count: 0`. Live request-envelope audit found the criterion-3 omission and is recorded outside Git at SHA-256 `4CE78DCA1E5674B47BA44145BDAB39D016A526546E33F245554927093E6E1B6D`; final corrected before/after proof remains pending. |
...
- No M14 dynamic adapters, runtime tracing, Ren'Py/game/creator execution, installer, hosted service,
  public distribution, media understanding, Q&A/chat, playthrough automation, editing, or patching.
...
- All visible milestone tasks use `gpt-5.6-sol` with High reasoning under repository dispatch policy.
  The creation surface has no fast-mode selector, so fast mode must be recorded unavailable/
  unverified rather than falsely claimed disabled.
- Every handoff includes exact base/head, branch/worktree, changed files, commands/results, reviewer
