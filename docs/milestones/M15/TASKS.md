...
| Core authority/storage seam correction | Coordinator | A1/A2 exact-authority audits and packet audit | `SourceSpan`, `StoryChunk`, and core contracts/generalized fixtures | Complete: failing-first tests proved non-choice event status, the compact mechanics packet, and validated mapper branch/scope meaning lacked durable seams. `SourceSpan` requires Python-owned reachability/warnings; `StoryChunk` requires canonical mechanics JSON; `CoreBranchOutcome`, per-chunk scope text, and assembled scope text retain validated provider contributions with deterministic anchors. Eight shared contract tests, Ruff, strict mypy, and whitespace pass; no guessed default was introduced. |
