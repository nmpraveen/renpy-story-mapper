...
| Core authority/storage seam correction | Coordinator | A1/A2 exact-authority audits | `SourceSpan` and core contracts/generalized fixtures | Complete: failing-first tests proved non-choice event status and validated mapper branch/scope meaning had no storage seam. `SourceSpan` now requires Python-owned reachability/warnings; `CoreBranchOutcome`, per-chunk scope text, and assembled scope text retain the validated provider contribution with deterministic anchors. Seven shared contract tests, Ruff, strict mypy, and whitespace pass; no guessed default was introduced. |
