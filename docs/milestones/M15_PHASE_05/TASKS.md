...
| Full regeneration and reader check | `/root/full_reextract_path`, `/root/whole_game_coverage_batch`, then at most two implementation workers plus Orchestra | Re-extract whole game, regenerate summaries/groups, reuse current grouped reader, inspect real result | One-slice `PASS` | In progress | Old 425/24 artifacts are baseline only; smallest existing rerun path is being identified |
