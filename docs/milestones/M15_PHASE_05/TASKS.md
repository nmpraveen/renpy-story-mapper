...
| Semantic review | One Sol/High reviewer | Check the extraction-first scope, authority split, exclusions, and lean evidence | Corrected contract | Pending | Review required before product-code changes |
| Ren'Py input/runtime proof | `/root/renpy_runtime_probe` plus Orchestra | Locate matching SDK and game inputs; determine full-project versus lone-script fidelity; prepare disposable-copy command | Read-only discovery | In progress | Pending worker handoff and focused experiment |
