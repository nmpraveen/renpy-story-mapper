...
| Privacy, scale, and private acceptance | Primary | Full-corpus simulation, bounded live/private harness, immutability | Integrated product | Blocked | Provider-free private pass at `e0fd3bf`; one consented live run failed with 74 failed jobs/222 transient attempts/24 calls/zero tokens; replay did not run; `LIVE_ACCEPTANCE_FAILURE.md` |
...
| Integration and verification | Primary | Integrate recovery A then B; inspect diffs; combined focused checks; freeze runtime; then gated acceptance | Recovery workers complete | In progress | Recovery base `4e2bf7a`; prior runtime freeze `e0fd3bf` remains valid historical evidence until a new runtime head is frozen |
